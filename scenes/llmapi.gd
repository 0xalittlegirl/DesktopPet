...
func call_aliyun(prompt):
	...
	# API配置部分 - 从环境变量获取配置
	# 从环境变量获取API密钥
	var api_key = OS.get_environment("VOLCES_API_KEY")
	if api_key.is_empty():
		push_error("环境变量VOLCES_API_KEY未设置")
		return
	...
	var body = JSON.stringify({
		"model": OS.get_environment("VOLCES_MODEL_NAME") if !OS.get_environment("VOLCES_MODEL_NAME").is_empty() else "deepseek-r1-250120",
		"messages" : messages,
		"stream" : false
	})
	# 从环境变量获取API端点URL
	var url = OS.get_environment("VOLCES_API_URL") if !OS.get_environment("VOLCES_API_URL").is_empty() else "https://ark.cn-beijing.volces.com/api/v3/chat/completions"
	# 设置请求超时为60秒
